...
class CurveBuilder:
    ...
    def optimize_long_term_rates(self, initial_rates, tenor_dates):
        def objective_function(zero_rates, tenor, market_rate):
            tenor_date = self.get_tenor_date(tenor)
            temp_spline = self.build_cubic_spline(tenor_dates, zero_rates)
            fixed_legs = []
            float_legs = []

            payment_dates = []
            current_date = self.value_date
            while current_date < tenor_date:
                current_date += relativedelta(months=3)
                current_date = self._adjust_modified_following(current_date)
                payment_dates.append(current_date)

            for i, payment_date in enumerate(payment_dates):
                prev_date = payment_dates[i-1] if i > 0 else self.value_date
                payment_dcf = self.calculate_dcf(prev_date, payment_date)
                t = self.calculate_dcf(self.value_date, payment_date)

                zero_rate = temp_spline(t)
                df = np.exp(-zero_rate * t)

                fixed_rate = 4 * (np.exp(market_rate/4) - 1)
                fixed_legs.append(df * payment_dcf * fixed_rate)
                float_legs.append(df)

            implied_rate = (1 - float_legs[-1]) / sum(fixed_legs)
        ...
        for tenor, rate in self.market_rates.items():
            if not self.is_short_term(tenor):
                def single_objective(r):
                    optimized_rates[-1] = r
                    return objective_function(optimized_rates, tenor, rate)

                optimized_rates[-1] = newton(
                    single_objective,
                    x0=rate,
                    tol=1e-6,
                    maxiter=100
...
if __name__ == '__main__':
    ...
    curve = CurveBuilder.load_from_excel('/home/yjalil/code/ois/curve_builder/tests/ois_input.xlsx', value_date)
    curve.build_curve()
